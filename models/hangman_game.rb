class HangmanGame < ActiveRecord::Base
  ...
  def self.count_guesses(word, guess, num_guesses)
  ...
  def self.create_index_array(word, guess)
    i=0
    index_array = []
    while i < word.length
      if guess == word[i]
  ...
  def self.check_status(new_game_state, word, num_guesses, user_id)
    if new_game_state == word
      gameover = "win"
      win = Win.find_by(user_id: user_id)
